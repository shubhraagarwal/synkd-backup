      ...
      switch (response[0].status) {
        case 200:
          ...
          for (var i = 0; i < this.state.rooms.length; i++) {
            ...
            console.log(
              this.state.rooms[i]._id + " -> " + this.state.rooms[i].roomname
            );
          }
    ...
    if (loading) {
      ...
      body = (
        <IonGrid className="phome-grid">
          <IonRow className="phome-row">{btn}</IonRow>
        </IonGrid>
      );
    } else {
      body = (
        <IonGrid className="phome-grid">
          <IonRow className="phome-row">
            ...
            <IonCol className="phome-col ion-align-self-center" size="4">
              ...
              >
                <IonIcon icon={addOutline} className="addBtnIcon" />
              </IonButton>
              <br />
      ...
      <IonContent>
        ...
        <IonModal isOpen={this.state.showSetupRoomModal}>
          <SetupRoomModalComponentHolder
            ...
            onDidDismiss={() => {
              this.setState({ showSetupRoomModal: false });
              this.refresh();
            }}
          />
        </IonModal>

        <IonModal isOpen={this.state.showSideMenuOptionsModal}>
          <SideMenuOptionsComponentHolder
            component={{ componentType: "PROFILE_SETTINGS" }}
            onDidDismiss={() => {
              this.setState({ showSideMenuOptionsModal: false });
            }}
          />
        </IonModal>
      </IonContent>
