...
class GroupGrid extends React.Component {
  ...
  async getGroups() {
    ...
    switch (groups) {
      case null:
        console.log(
          "No groups found in local storage. Attempting to retrieve from cloud storage"
        );
        break;
      ...
      switch (response[0].status) {
        case 200:
          ...
          localStorage.setItem(
            "groups",
            JSON.stringify(response[0].data.groups)
          );
        ...
        default:
          this.setState({ showToast: true, toastMsg: "Server Error." });
          this.props.groups([]);
          break;
      }
    } else {
  ...
  async setGroupState(groupid) {
    ...
    for (var i = 0; i < groupSlotCount; i++) {
      if (
        (await this.getSlotState(
          group.slots[i].slotnumber,
          group.slots[i].mac
        )) === 0
      ) {
        groupStateInt++;
